      ...
      const resp = await fetch(
        ...
        attachGateHeaders({
          ...
          body: JSON.stringify({
            ...
            execution_strategy: extra?.executionStrategy ?? 'auto',
          }),
...
export interface ContainerTaskCreateBody {
  image: string;
  command: string[];
  timeout_sec?: number;
  cpu?: number;
...
export interface ContainerTaskPublic {
  task_id: string;
  status: string;
  image: string;
  command: string[];
...
export class ContainerTaskWebSocket {
  ...
  connect(): void {
    ...
    this.ws.onmessage = (event) => {
      try {
        this.onMessage(JSON.parse(event.data) as TaskWsMessage);
      } catch {
        /* ignore */
      }
    };
