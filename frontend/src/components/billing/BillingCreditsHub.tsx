/**
 * 靈境積分中心 — 單頁滾動：總覽 / 雲與 Docker / 積分池 / 貢獻 / 申訴 / 管理
...
const ANCHOR_IDS = CREDITS_SECTIONS.map((s) => s.anchorId);

export default function BillingCreditsHub() {
  const scrollRef = useRef<HTMLDivElement>(null);
  const scrollTo = useScrollToSection(scrollRef);
  const activeId = useSectionScrollSpy(ANCHOR_IDS, scrollRef);
  ...
  const focusSection = useCallback(
    (section: CreditsSectionKey) => {
      const anchor = creditsAnchorId(section);
  ...
  useEffect(() => {
    focusSection(parseCreditsSection());
  }, [focusSection]);

  useEffect(() => {
    const onHash = () => focusSection(parseCreditsSection());
    window.addEventListener('hashchange', onHash);
    return () => window.removeEventListener('hashchange', onHash);
  }, [focusSection]);

  const onNavSelect = (anchorId: string) => {
    const section = CREDITS_SECTIONS.find((s) => s.anchorId === anchorId);
    if (section) jumpToCreditsSection(section.key);
    scrollTo(anchorId);
  };
  ...
  return (
    <PanelShell scroll={false}>
      <ConsoleSectionNav sections={NAV_ITEMS} activeId={activeId} onSelect={onNavSelect} />
      ...
      <PanelScroll ref={scrollRef}>
        <PanelSection className={consoleLayout.sectionGapLg}>
          <SectionHeader
            title="靈境積分中心"
            description="積分帳務、Docker 按時計費、阿里雲 BSS、貢獻鎖倉與管理台 — 同一頁連續瀏覽"
