...
import { PanelAlert, PanelScroll, PanelShell, SectionHeader } from './ui/ConsoleLayout';
  ...
  return (
    <PanelShell scroll={false}>
      <div className="shrink-0 border-b border-white/[0.08] px-6 py-4">
        ...
        />
        {error ? <PanelAlert className="mt-3">{error}</PanelAlert> : null}
      </div>

      <div className="flex shrink-0 flex-wrap items-center gap-x-4 gap-y-1 border-b border-white/[0.06] px-6 py-2 text-[11px]">
      ...
      {ops?.last_error && (
        <p className="shrink-0 px-6 pt-2 text-[12px] text-amber-200">
