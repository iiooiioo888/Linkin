...
**術語：** [docs/glossary.md](../docs/glossary.md)  
...
| 任務／軌跡／監控 | `services/task_manager.py`、`trace_logger.py`、`monitor_hub.py` |

| 模型目錄爬取 | `services/llm_ops.py` |

| 登入閘門 | `auth/` |

| 種子／煙測／匯出 | `scripts/`（見下方） |

| 測試 | `tests/`（一律 `monkeypatch`，不連真實 LLM／遊戲） |



角色數字：**85**＝公司 `STANDARD_ROLES`；**16**＝靈境子角色。勿混用。  
詳表：[角色介紹](../docs/company/roles.md) · [structure.md](../docs/structure.md)。
...
| `python -m backend.scripts.export_roles_doc` | 匯出角色介紹 → `docs/company/roles.md` |
